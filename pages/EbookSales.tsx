                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-end gap-3 border-t border-gray-50 pt-3 min-w-0">
                    <div className="flex flex-col min-w-0 flex-1 overflow-hidden">
                      ...
                      <span className="text-[11px] font-black text-gray-600 italic break-words">{members.find(m => m.id.toLowerCase() === (ebook.authorId ?? '').toLowerCase())?.nickname || ebook.author}</span>
                    </div>
                    <div className="flex flex-col items-end shrink-0 gap-1">
                      {(() => { const u = members.find(m => m.id.toLowerCase() === (ebook.authorId ?? '').toLowerCase() || m.nickname === ebook.author); const g = getUserGrade(u, gradeConfigs); return g ? (
