...
const ChannelSales: React.FC<Props> = ({ channels, wishlist, onToggleWishlist }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4">
      <div className="bg-white p-6 md:p-10 rounded-[40px] shadow-sm mb-12 border border-gray-100">
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {['전체', 'YouTube', 'TikTok', 'Twitter', 'Instagram', 'Facebook', 'Telegram'].map((p) => (
            <button key={p} onClick={() => setActivePlatform(p)} className={`px-6 py-2 rounded-full text-[13px] font-black transition-all border-2 ${activePlatform === p ? 'bg-gray-900 text-white border-gray-900 shadow-md scale-105' : 'bg-white text-gray-400 border-gray-100 hover:border-gray-200'}`}>{p}</button>
          ))}
        </div>
        <div className="max-w-5xl mx-auto space-y-4">
          <div className="flex flex-col md:flex-row gap-3">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="채널명으로 검색" className="flex-[2] p-4 bg-gray-50 border border-transparent rounded-2xl outline-none font-bold text-gray-700 focus:bg-white focus:border-blue-500 transition-all shadow-inner text-[15px]" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-4 border-t border-gray-50">
            <div className="flex items-center gap-3"><span className="text-[11px] font-black text-gray-400 shrink-0 w-12 uppercase italic tracking-widest">Subs</span><div className="flex-1 flex items-center gap-2"><input type="number" placeholder="부터" value={minSubs} onChange={(e) => setMinSubs(e.target.value)} className="w-full p-3 bg-gray-50 border border-transparent rounded-xl outline-none font-bold text-gray-700 text-xs focus:bg-white focus:border-gray-200 shadow-inner" /><span className="text-gray-300">~</span><input type="number" placeholder="까지" value={maxSubs} onChange={(e) => setMaxSubs(e.target.value)} className="w-full p-3 bg-gray-50 border border-transparent rounded-xl outline-none font-bold text-gray-700 text-xs focus:bg-white focus:border-gray-200 shadow-inner" /></div></div>
            <div className="flex items-center gap-3"><span className="text-[11px] font-black text-gray-400 shrink-0 w-12 uppercase italic tracking-widest">Price</span><div className="flex-1 flex items-center gap-2"><input type="number" placeholder="부터" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} className="w-full p-3 bg-gray-50 border border-transparent rounded-xl outline-none font-bold text-gray-700 text-xs focus:bg-white focus:border-gray-200 shadow-inner" /><span className="text-gray-300">~</span><input type="number" placeholder="까지" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="w-full p-3 bg-gray-50 border border-transparent rounded-xl outline-none font-bold text-gray-700 text-xs focus:bg-white focus:border-gray-200 shadow-inner" /></div></div>
            <div className="flex items-center gap-3"><span className="text-[11px] font-black text-gray-400 shrink-0 w-12 uppercase italic tracking-widest">Inc.</span><div className="flex-1 flex items-center gap-2"><input type="number" placeholder="부터" value={minIncome} onChange={(e) => setMinIncome(e.target.value)} className="w-full p-3 bg-gray-50 border border-transparent rounded-xl outline-none font-bold text-gray-700 text-xs focus:bg-white focus:border-gray-200 shadow-inner" /><span className="text-gray-300">~</span><input type="number" placeholder="까지" value={maxIncome} onChange={(e) => setMaxIncome(e.target.value)} className="w-full p-3 bg-gray-50 border border-transparent rounded-xl outline-none font-bold text-gray-700 text-xs focus:bg-white focus:border-gray-200 shadow-inner" /></div></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 px-2">
        {filteredChannels.length === 0 ? (
          <div className="col-span-full py-32 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100">
             <p className="text-gray-300 font-black italic text-lg">조건에 맞는 채널이 없습니다.</p>
          </div>
        ) : filteredChannels.map((ch) => (
          <div key={ch.id} className="bg-white rounded-[24px] overflow-hidden shadow-sm group border border-gray-100 relative transition-all hover:-translate-y-2 hover:shadow-xl">
            <Link 
