...
const MyPage: React.FC<Props> = ({ user, onUpdate, ebooks, setEbooks, channels, smmOrders, channelOrders, storeOrders, onAddReview, onUpdateReview, reviews, addNotif, onRefetchProfile, gradeConfigs = [] }) => {
  ...
  const availableCouponCount = useMemo(() => {
    return user.coupons?.filter(c => c.status === 'available').length || 0;
  }, [user.coupons]);
  ...
  const handleProfileImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      onUpdate({ ...user, profileImage: reader.result as string });
    };
  ...
  const handleNicknameUpdate = () => {
    ...
    onUpdate({ ...user, nickname: editNicknameValue.trim() });
    ...
    <div className="max-w-[1400px] mx-auto pb-32 px-4 lg:px-8 space-y-10 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-10 rounded-[48px] shadow-sm border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className="relative group w-28 h-28 cursor-pointer shrink-0" onClick={() => fileInputRef.current?.click()}>
            <img src={user.profileImage} className="w-full h-full rounded-[36px] object-cover border-4 border-blue-50 shadow-xl" alt="profile" />
            ...
            {isEditingNickname ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => { setIsEditingNickname(false); setEditNicknameValue(user.nickname); setNicknameStatus('idle'); }} className="px-3 py-2 bg-gray-50 text-gray-400 rounded-lg text-[11px] font-black">X</button>
                </div>
              </div>
            ) : (
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <div className="bg-gray-50 p-6 rounded-[32px] flex items-center gap-6 min-w-[200px]">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-gray-900 italic">{(user.points || 0).toLocaleString()} P</h3>
            </div>
      ...
      <div className="bg-gray-100/50 p-2 rounded-[32px] flex flex-wrap gap-2 w-full max-w-5xl mx-auto shadow-inner">
        {[
          ...
          >
            {mode.label}
            {mode.id === 'seller' && user.sellerStatus !== 'approved' && <span className="ml-2 opacity-50">🔒</span>}
          </button>
        ))}
      </div>
