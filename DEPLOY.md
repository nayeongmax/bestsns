...
### 6-4. 탈퇴한 계정인데 로그인/회원가입 안 했는데 users·profiles에 계정이 다시 생기는 경우

**원인:** 탈퇴한 뒤 **「구글로 로그인」** 또는 **「카카오로 로그인」** 버튼을 누르면, Supabase는 “기존 계정 없음”으로 보고 **새 사용자(auth.users + 트리거로 profiles)** 를 만들어 버립니다. 그래서 “회원가입은 안 했는데”라고 느껴도, **로그인 탭의 소셜 로그인 한 번**이 새 계정 생성으로 이어질 수 있습니다.

**대응:** 앱에서 “로그인” 의도로 소셜 연동했는데, 방금 생성된 계정(프로필 생성 2분 이내)이면 **해당 계정을 자동 삭제**하도록 되어 있습니다. 따라서 탈퇴한 이메일로 실수로 소셜 로그인만 눌러도 users·profiles에 허공 계정이 쌓이지 않습니다. 재가입을 원하면 **회원가입** 탭에서 **구글로 회원가입** / **카카오로 회원가입**을 진행하면 됩니다.
