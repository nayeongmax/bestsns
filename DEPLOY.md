...
- **실제 로그인**은 Supabase **`profiles`** 테이블에서 **아이디 → 이메일**을 조회한 뒤 `signInWithPassword(이메일, 비밀번호)`로 처리합니다.  
  → 따라서 **`profiles`에 행이 없거나, 테이블 구조가 다르면** 다른 기기에서 로그인이 되지 않습니다.
...
1. Supabase **SQL Editor** → New query → **`supabase-profiles-setup.sql`** 전체 내용을 붙여넣고 **Run**으로 실행하세요.  
2. 이미 다른 구조로 만든 `profiles` 테이블이 있다면, 컬럼명을 위와 맞추거나 테이블을 삭제 후 다시 `supabase-profiles-setup.sql`로 생성하세요.  
3. 회원가입·로그인 성공 시 여기에 자동으로 upsert됩니다. Table Editor에서 **id, email, nickname**이 보여야 정상입니다.  
...
로그인 화면의 **네이버 로그인**, **카카오 로그인** 버튼을 사용하려면 Supabase에서 해당 프로바이더를 활성화해야 합니다.

1. [Supabase 대시보드](https://supabase.com/dashboard) → 프로젝트 선택
2. **Authentication** → **Sign In / Providers** 이동
3. **Kakao** 또는 **Naver** 행을 펼친 뒤 **Enable** 켜기
...
- **네이버:** [Naver Developers](https://developers.naver.com) → 애플리케이션 등록 → API 설정에서 로그인 API 사용, Callback URL에 Supabase 제공 URL 등록
