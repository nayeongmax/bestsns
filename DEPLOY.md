...
소셜 로그인으로 가입했는데 **Table Editor → profiles**에 행이 안 보이면, **RLS(행 수준 보안)**에서 **INSERT**가 허용되지 않았을 가능성이 큽니다.
...
3. 또는 **`supabase-rls-profiles-only.sql`** / **`supabase-rls-public-all-site-data.sql`** 을 **다시 한 번** 실행해 주세요. (두 파일 모두 profiles에 대한 **INSERT** 정책이 포함되어 있습니다.)
4. 이후 소셜로 새로 가입한 사용자는 **profiles**에 자동으로 추가되고, 어드민 회원 목록·Table Editor에 나타납니다.
