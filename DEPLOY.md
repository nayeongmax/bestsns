...
### 5-2. 소셜(구글/카카오) 회원가입한 계정이 profiles에 아예 안 뜨는 경우

**증상:** 구글이나 카카오로 회원가입했는데 **Table Editor → profiles**에 행이 전혀 안 보임. (삭제가 잘 되는 게 아니라, 소셜 가입 계정이 처음부터 profiles에 안 들어감.)  
**원인:** auth.users에는 생기지만, profiles에 행을 넣는 설정(트리거 또는 RLS INSERT)이 적용되지 않음.  
**해결:** 아래 **방법 A(트리거)** 를 먼저 적용하세요. RLS와 관계없이 DB에서 자동으로 행이 추가됩니다.

#### 방법 A (권장): Auth 트리거로 자동 추가

구글/카카오 등 **auth.users에 새 사용자가 생성될 때** DB에서 자동으로 **profiles**에 한 행을 넣는 트리거를 켜면, RLS 설정과 상관없이 항상 행이 생깁니다.

1. **Supabase 대시보드** → **SQL Editor** → 새 쿼리
2. 프로젝트 폴더의 **`supabase-auth-profiles-trigger.sql`** 파일 내용 **전체**를 복사해 붙여넣기
3. **Run** 실행
4. 이후 **새로** 구글로 회원가입한 사용자부터 **profiles**에 자동으로 추가됩니다. (이미 가입한 계정은 아래 백필 스크립트로 추가 가능)
