...
const Header: React.FC<Props> = ({ user, wishlistCount, notifications, unreadChatCount, onLogout, onOpenLoginModal }) => {
  ...
  return (
    <>
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        ...
        <div className="hidden xl:block">
          <div className="container mx-auto px-4 h-20 flex items-center justify-between max-w-[1550px]">
            ...
            <nav className="flex items-center gap-1 flex-1 justify-center h-full">
              {navItems.map((item) => {
      ...
      >
        <div className="absolute inset-0 bg-black/50" onClick={closeMobileMenu} aria-label="메뉴 닫기" />
        <div
        ...
        >
          <div className="flex items-center justify-between px-4 h-16 border-b border-gray-100 shrink-0">
            ...
            <button type="button" onClick={closeMobileMenu} className="p-2 rounded-xl text-gray-500 hover:bg-gray-100 hover:text-gray-900" aria-label="메뉴 닫기">
          ...
          <nav className="flex-1 overflow-y-auto py-4">
            ...
            <div className="flex items-center gap-2 px-5 pt-2 pb-4 sm:pb-5 border-b border-gray-100">
              ...
              <Link to="/chat" onClick={closeMobileMenu} className="flex-1 flex items-center justify-center gap-2 py-3.5 sm:py-4 rounded-xl bg-gray-50 text-gray-600 font-black text-sm sm:text-base hover:bg-blue-50 hover:text-blue-600 transition-colors">
                ...
                <span>채팅</span>
                {unreadChatCount > 0 && <span className="bg-blue-500 text-white text-[10px] sm:text-xs px-1.5 py-0.5 rounded-full font-bold">{unreadChatCount > 99 ? '99+' : unreadChatCount}</span>}
              </Link>
