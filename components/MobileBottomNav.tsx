        ...
        {navItems.map((item) => {
          ...
          return (
            <a
              key={item.path}
              href={href}
              className={`relative flex flex-col items-center justify-center min-w-[85px] h-16 transition-all duration-300 flex-shrink-0 pb-2 ${isActive ? 'text-blue-600 scale-105' : 'text-gray-400'}`}
            >
              {item.badge && (
